...
//		Prefs.with(context).save(Constants.SHOW_AGREEMENT_SCREEN, userData.optInt("agreement_status", 1));
        ...
        Type listType = new TypeToken<List<DriverVehicleServiceTypePopup.VehicleServiceDetail>>() {
        }.getType();
    ...
    public String parseCurrentUserStatus(Context context, JSONObject jObject1) {
        ...
        try {
            if (jObject1.has(KEY_ERROR)) {
                ...
                if (Data.userData != null && jObject1.has("driverDestination")) {
                    JSONObject destRideObj = jObject1.getJSONObject("driverDestination");
                    if (destRideObj.has(Constants.KEY_ADDRESS)) {
                        Data.userData.currDestRideObj = Data.userData.new CurrDestRide(destRideObj.getString(Constants.KEY_ADDRESS), destRideObj.getDouble("destination_latitude"), destRideObj.getDouble("destination_longitude"), destRideObj.getInt("destination_ride_time_remaining"), System.currentTimeMillis(), destRideObj.getString("type"));
                        destRideObj.optInt("destination_ride_time_remaining", 0);
                    }
                }
