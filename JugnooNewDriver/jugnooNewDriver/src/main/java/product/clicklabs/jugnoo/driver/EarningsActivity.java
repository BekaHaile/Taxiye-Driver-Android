...
//				MyApplication.getInstance().logEvent(EARNING + "_" + RIDE_HISTORY, null);
		...
		if(Data.isCaptive()){
			...
			getEarningsDetails(this, 0);
			llGraphWithEarnings.setVisibility(Prefs.with(this).getInt(Constants.KEY_SHOW_GRAPH_IN_EARNINGS, 1) == 1 ? View.VISIBLE : View.GONE);
...
//	Retrofit
	...
	public void updateData(DriverEarningsResponse driverEarningsResponse, boolean walletClick) {
		...
		if (driverEarningsResponse != null) {
			...
			if(Prefs.with(this).getInt(Constants.KEY_SHOW_GRAPH_IN_EARNINGS, defaultVisibility) != 1){
				return;
			}
