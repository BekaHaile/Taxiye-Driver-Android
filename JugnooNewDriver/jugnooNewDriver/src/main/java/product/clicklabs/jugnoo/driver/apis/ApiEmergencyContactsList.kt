 ...
 */
class ApiEmergencyContactsList(private val activity: Activity, private val callback: Callback) {

    private val TAG = ApiEmergencyContactsList::class.java.simpleName
    ...
    fun emergencyContactsList() {
        try {
            if (MyApplication.getInstance().isOnline) {
                ...
                RestClient.getApiServices().emergencyContactsList(params, object : retrofit.Callback<SettleUserDebt> {
                        ...
                        try {
                            ...
                            if (!SplashNewActivity.checkIfTrivialAPIErrors(activity, jObj, flag, null)) {
                                if (ApiResponseFlags.EMERGENCY_CONTACTS.getOrdinal() == flag) {
                                    Data.userData.emergencyContactsList = JSONParser.parseEmergencyContacts(jObj)
                                    callback.onSuccess()
                                } else {
